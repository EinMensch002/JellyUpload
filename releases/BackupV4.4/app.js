 ...
 * Entfernt ALLE bekannten Video-Dateierweiterungen
 * Ignoriert URL-Suffixe wie ".to", ".la", ".net", etc. die NACH Video-Dateitypen vorkommen
 * 
 * WICHTIG: Erkennt nur ECHTE Video-Dateierweiterungen, nicht beliebige Punkte
 * 
 * Beispiele:
 * "Movie.mp4" → "Movie"
 * "Movie.to.mp4" → "Movie" (erkennt .mp4 und ignoriert .to davor)
...
function removeFileExtension(filename) {
  ...
  // Fallback: Wenn keine bekannte Erweiterung gefunden, entferne den letzten .Punkt
  // (z.B. für unbekannte Formate)
...
function editEpisodeModal(button) {
  ...
  // Sammle alle existierenden Serien
        ...
        <div class="edit-modal-body">
          ...
          <div class="form-group">
            ...
            <div style="display: flex; gap: 8px; margin-bottom: 12px;">
              <select class="series-edit" data-filename="${escapeHtml(filename)}" style="flex: 1;">
                <option value="${escapeHtml(data.series_name)}" selected>${escapeHtml(data.series_name)}</option>
              ...
              <button class="btn-secondary" onclick="addNewSeriesInModal(this)" style="padding: 8px 12px; white-space: nowrap;">+ Neue Serie</button>
            </div>
            <p style="font-size: 0.85rem; color: #6b7280; margin: 0;">💡 Wähle eine andere Serie oder erstelle eine neue, um die Episode dorthin zu verschieben.</p>
          </div>
 ...
 * Sucht nach der Datei in Episodes, Filmen oder sonstigen Items und updated Badges/Namen
 */
function updateFileUIAfterEdit(filename) {
  ...
  // Merge: Original-Daten + Edits
  const currentName = edits.jellyfin_name || originalData.jellyfin_name || '';
  const currentFsk = edits.fsk || originalData.fsk || '';
  const currentAudience = edits.audience || originalData.audience || 'unknown';
  const currentSeason = edits.season !== undefined ? edits.season : originalData.season;
  const currentEpisode = edits.episode !== undefined ? edits.episode : originalData.episode;
  ...
  if (!fileElement) {
    fileElement = document.querySelector(`.movie-row[data-filename="${escapeHtml(filename)}"]`);
  }
  
  // Strategie 3: Suche nach other-item (für sonstige)
  if (!fileElement) {
    fileElement = document.querySelector(`[data-filename="${escapeHtml(filename)}"]`);
  }
  
  if (!fileElement) {
    logDebug(`⚠️ Konnte Element für ${filename} nicht finden`, 'warning');
    return;
  }
  
  // Update Name/Titel - versuche mehrere Selektoren
  ...
  if (!nameElement) nameElement = fileElement.querySelector('.other-title');
  
  if (nameElement && currentName) {
    nameElement.textContent = currentName;
    logDebug(`📝 Name aktualisiert: ${currentName}`, 'data');
  }
  
  // Update FSK-Badge - finde oder erstelle
  let fskBadge = fileElement.querySelector('.badge-fsk');
  if (currentFsk) {
    if (fskBadge) {
      fskBadge.textContent = `FSK ${currentFsk}`;
    } else {
      // Erstelle neues FSK-Badge
      const episodeMeta = fileElement.querySelector('.episode-meta') || 
                         fileElement.querySelector('.movie-meta') ||
                         fileElement.querySelector('.file-meta');
      if (episodeMeta) {
    ...
    logDebug(`🎯 FSK aktualisiert: FSK ${currentFsk}`, 'data');
  }
  ...
  if (audienceBadge) {
    ...
    const label = (currentAudience === 'kids' || currentAudience === 'children') ? 'Kinder' : 'Erwachsene';
    audienceBadge.textContent = icon;
    audienceBadge.className = `badge-audience ${currentAudience}`;
    audienceBadge.title = label;
    logDebug(`👥 Audience aktualisiert: ${label}`, 'data');
  }
  
  // Update Episode-Info (für Serien)
...
function saveSuggestion(overlay, filename) {
  ...
  // 🎨 Update UI statt komplett neu zu rendern
  updateFileUIAfterEdit(filename);
}
...
function saveSeriesSuggestion(overlay, oldSeriesName) {
  ...
  Object.entries(STATE.analysisResults).forEach(([filename, result]) => {
    if (result.series_name === oldSeriesName) {
      ...
      // 🎨 Update UI für jede Episode
      updateFileUIAfterEdit(filename);
    }
  });
  
  logDebug(`✏️ Seriename aktualisiert: ${newSeriesName}`, 'info');
  closeEditModal(overlay);
}
...
function saveSeriesRename(overlay, oldSeriesName) {
  ...
  // 5. 🎨 FIX: Aktualisiere auch die Reiter-Überschrift
  // Suche nach dem series-entry-title für die alte Serie
