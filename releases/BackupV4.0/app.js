...
function displayAnalysisResults() {
  ...
  if (otherItems.length > 0) {
    // Trennung: Normale Items vs. Fehler
    const normalOther = otherItems.filter(item => item.status !== 'error' && item.status !== 'failed');
    const errorOther = otherItems.filter(item => item.status === 'error' || item.status === 'failed');
    
    htmlContent += `
      <div class="analysis-group">
        <div class="analysis-group-header">❓ Sonstiges & Fehler (${otherItems.length})</div>
        <div class="analysis-overview">
    `;
    
    // Normale Items zuerst
    if (normalOther.length > 0) {
      htmlContent += normalOther.map((item) => `
    ...
    // Fehlerhafte Items separat mit Warning
    if (errorOther.length > 0) {
      htmlContent += `
        <div style="margin-top: 16px; padding-top: 16px; border-top: 2px solid #fee2e2;">
          <div style="background: #fef2f2; padding: 12px; border-radius: 6px; margin-bottom: 12px; border-left: 4px solid #dc2626;">
            <strong style="color: #dc2626;">⚠️ Nicht erkannte Dateien (${errorOther.length})</strong>
            <p style="margin: 8px 0 0 0; color: #6b7280; font-size: 0.9rem;">
              Diese Dateien konnten von der AI nicht erkannt werden. Sie können diese manuell bearbeiten oder ignorieren.
            </p>
          </div>
          ${errorOther.map((item) => `
            <div class="error-file-item" data-filename="${escapeHtml(item.filename)}" style="
              background: #fafafa;
              padding: 12px;
              margin-bottom: 8px;
              border-radius: 4px;
            ...
            ">
              <div style="flex: 1;">
                <strong style="color: #dc2626;">📄 ${escapeHtml(item.original_name)}</strong>
                <p style="margin: 4px 0 0 0; color: #6b7280; font-size: 0.85rem;">
                  ${escapeHtml(item.message || 'Fehler: Datei konnte nicht analysiert werden')}
                </p>
              </div>
              <button class="btn-edit-other" data-filename="${escapeHtml(item.filename)}" onclick="editOtherModal(this)" style="margin-left: 12px;">✎ Bearbeiten</button>
            </div>
          `).join('')}
        </div>
      `;
