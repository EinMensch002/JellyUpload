...
2. Debug-Panel zeigt jetzt **expandbare Details**
...
### Scenario 1: 3 Downloads mit je 2 Folgen
```
Download 1: One Piece S1 E1, S1 E2
Download 2: One Piece S1 E3, S1 E4  
Download 3: One Piece S1 E5, S1 E6

Workflow:
1. Alle hochladen + analysieren
2. Alle 6 Episoden sind ausgewählt
3. Prüfe: "Habe ich jetzt alle Folgen der Staffel?"
4. Ja → Finalisiere alle
5. Nein → Deselektiere fehlende Episodes & finalisiere nur die
```

### Scenario 2: Filme mit FSK-Filter
```
3 Filme hochgeladen:
- Inception (alle Zielgruppen)
- Barbie (nur Kinder)
- Hostel (nur Erwachsene)

Workflow:
1. Alle analysieren
2. Klick auf "Filme"-Bereich
3. Bulk-Set: "👨 Erwachsene" für Inception+Hostel
4. Edit Barbie: "👶 Kinder"
5. Finalisieren
```
...
*(Shortcuts sind optional - Buttons funktionieren immer)*
...
1. **Batch-Processing:** 
   - Lade mehrere Dateien auf einmal hoch
   - Analysiere gemeinsam
   - Finalisiere nur needed Dateien

2. **Session speichern:**
   - SessionId wird in Debug-Panel angezeigt
   - Kann für API-Calls verwendet werden

3. **Logs exportieren:**
   - 🐛-Button → "Export Logs"
   - Speichert als JSON für Debugging

4. **Seite auffrischen:**
   - Nach Finalisierung: Automatisch
   - Manuell: F5 oder Browser-Refresh

---

## 🚨 Wichtig: Dateiendungen

Die App sendet Dateiendungen (.mp4, .mkv, etc.) als separate Variable:
```json
{
  "originalName": "Episode 1.mp4",
  "fileExtension": ".mp4",  ← Automatisch extrahiert!
  ...
}
```
...
- **Vollständige Doku:** `README_V4.0.md`
- **Server Setup:** `serverbackend V2.0.json`
- **AI-Integration:** `serverbackend aicluster V1.1.5.json`
- **Changelog:** `CHANGELOG_V4.0.md`
...
A: Abhängig vom Server. DB-Cache: ~1sec. AI-Analyse: ~5-30sec.

**F: Kann ich Dateien während Upload löschen?**
A: Ja, klick auf ❌ vor dem Upload startet.

**F: Was passiert wenn N8N offline ist?**
A: Error-Status wird angezeigt, erneut probieren wenn verfügbar.

**F: Kann ich die Seite schließen während Finalisierung?**
A: Nicht empfohlen! Requests laufen noch weiter.

---

**Du bist bereit! Los geht's! 🚀**

*Viel Spaß mit Jellyfin Media Sortierung v4.0!*
